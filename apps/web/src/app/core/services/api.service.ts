...
export class ApiService {
  ...
  private get<T>(endpoint: string, params?: Record<string, unknown>): Observable<ApiResponse<T>> {
  ...
  private post<T>(endpoint: string, body: unknown): Observable<ApiResponse<T>> {
  ...
  private put<T>(endpoint: string, body: unknown): Observable<ApiResponse<T>> {
  ...
  getPaginated<T, P extends PaginationParams = PaginationParams>(
    endpoint: string,
    params?: P
  ): Observable<PaginatedResponse<T>> {
    return this.get<PaginatedResponse<T>>(endpoint, params as Record<string, unknown>).pipe(
